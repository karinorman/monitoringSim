...
get_eqprob <- function(pt_df, n, pik, aux_df) {
  ...
  lpm2_fit <- BalancedSampling::lpm2(pik, pt_mat)
    ...
    #Cube sampling
    cube_fit <- BalancedSampling::cube(pik, pt_aux)

    #SCPS
    scps_fit <- BalancedSampling::scps(pik, pt_aux)

    #LPM
    lpm1_fit <- BalancedSampling::lpm1(pik, pt_aux)
    lpm2_fit <- BalancedSampling::lpm2(pik, pt_aux)
