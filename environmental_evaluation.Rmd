...
envdata <- envdata_pts %>% 
  dplyr::select(-ID, -can_lc_proj) %>%
  drop_na()

env_pca <- ade4::dudi.pca(df = envdata, 
...
envdata_samp <- sample_n(as.data.frame(env_pca$li), 500)
...
sim_df <- map_df(samp_props, get_prop_overlap, comp_hyp = comp_hyp, env_data = as.data.frame(env_pca$li)) %>%
  cbind(samp_props, .)
