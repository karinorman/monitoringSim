...
sim_dist <- left_join(samplesize_sim, binned_data, by = c("id" = "ID")) %>% 
  group_by(simid) %>% 
  count(bins) %>% 
  pivot_wider(names_from = bins, values_from = n, values_fill = 0) %>%
...
```{r}
get_var_jsd <- function(data, var_name){
  binned_data <- data %>% 
  select(ID, !!var_name) %>%
...
comp_total <- select(jsd, simid, !!sym(var_name)) %>%
  filter(simid != var_name)
}
...
env_vars_jsd <- map(env_vars, ~get_var_jsd(data = envdata, var_name = .x)) %>% 
