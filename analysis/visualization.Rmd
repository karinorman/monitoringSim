...
plot_metric_curves <- function(data, metric_var, type, ylabel, xlabel, aux, exclude_algos = NULL, color_list, plot_path = NULL, ...){
  plot <- data %>%
    select(sample_size, algorithm_name, stratified, !!metric_var, aux_vars) %>%
    filter(stratified == type, aux_vars == aux) %>%
    ggplot(aes(x = sample_size, y = !!sym(metric_var), color = algorithm_name)) +
...
noaux_plot_list <- purrr::pmap(noaux_plot_df, plot_metric_curves, data = metrics, aux = FALSE, color_list = algo_colors, axis.title = element_text(size=7))
...
legend_plot <- metrics %>%
    ...
    theme_classic() +
    theme(axis.title = element_text(size=7)) +
  scale_colour_manual(values = algo_colors, drop = TRUE)
