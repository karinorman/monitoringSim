...
get_eqprob_strat <- function(pt_df, pt_mat, pik, n, N, strata_n){
  ...
  #### What of the commenting below needs to be moved out?? ####
  # 
  ...
  # CUBE
  cube_strat <- BalancedSampling::cubestratified(prob = pik, Xbal = strat_df, integerStrata = inc_pts$ECO_NUM)
...
get_strata_reps <- function(n, N, strat_df, nreps, strata_col, area_col) {
  ...
  pt_mat <- as.matrix(cbind(sf::st_coordinates(strat_df)[,1],
                            sf::st_coordinates(strat_df)[,2]))
