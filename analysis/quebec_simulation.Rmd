...
get_eqprob_strat <- function(pt_df, pt_mat, pik, n, N, strata_n){
...
```{r}
get_strata_reps <- function(n, N, strat_df, nreps, strata_col, area_col) {
    
  #get count of number of points in each stratum
  stratum_counts <- strat_df %>%
    count({{strata_col}}, name = "N") %>%
    st_drop_geometry()
  ...
  prop_samp <- strat_df %>% 
    ...
    st_drop_geometry() %>%
    distinct() %>%
    mutate(sample_size = as.integer(round({{area_col}} * n))) %>%
    left_join(stratum_counts) %>%
    mutate(inc_prob = sample_size/N)
  ...
  if(dim(missing_ecoregion)[1] > 0){
    warning(paste0("There are ", dim(missing_ecoregion)[1], " ecoregions that have not been allocated any samples."))
  ...
  purrr::map_dfr(1:nreps,
                 function(rep) {
                   ...
                   get_eqprob_strat(pt_df = ecoregion_pts, pt_mat = pt_mat, pik = pik,
                              n = n, N = N, strata_n = strata_n) %>%
                     mutate(rep = rep)
                 })
}

purrr::map_dfr(sample_sizes[2:3], ~get_strata_reps(.x, N = N, strat_df = inc_pts, 
