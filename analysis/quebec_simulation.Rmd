...
strata_sim <- furrr::future_map(sample_sizes, 
                                        ~get_strata_reps(.x, strat_df = ecoregion_points_data, 
                                                    nreps = repn, strata_col = ECO_ID, area_col = PERC_AREA), 
                                        .options = furrr_options(seed = TRUE))
...
### Using Environmental data as auxillary variables
```{r}
...
eqprob_aux <- map_dfr(sample_sizes, ~get_eqprop_reps(.x, N = N, pt_df = qc_cand_points, nreps = repn, aux_df = env_red) %>% 
                                          mutate(sample_size = .x))

eqprob_strata_aux <- map(sample_sizes[1:3], ~get_strata_reps(.x, strat_df = ecoregion_points_data, 
