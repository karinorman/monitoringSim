...
euc_dist <- st_distance(qc_cand_points %>% filter(id %in% specieslist_wide$id))
euc_dist_df <- as_tibble(rstatix::replace_lower_triangle(as.matrix(euc_dist), by = NA)) %>%
  select(-rowname)

colnames(euc_dist_df) = specieslist_wide$id
euc_dist_df <- euc_dist_df %>%
  mutate(id = specieslist_wide$id) %>%
  pivot_longer(-id, names_to = "colid", values_to = "euc") %>%
  filter(!is.na(euc))

dist_df <- left_join(beta_dist_df, euc_dist_df) %>%
  mutate(euc = as.numeric(euc))

usethis::use_data(dist_df)

# dist_df %>%
#   ggplot(aes(x = jaccard, y = euc)) +
#   geom_point()
